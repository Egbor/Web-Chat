...
class WCSignUp extends React.Component {
    componentDidMount() {
        let checker = new FormChecker();
        /*Check events*/
        checker.add("login", element => element.value !== "");
        checker.add("password", element => element.value !== "");
        checker.add("rep-password", element => element.value === document.getElementById("password").value);
        /*Check callbacks*/
    ...
    render() {
        return(
            <div className="wcauth-wrapper">
                ...
                <div className="mb-3">
                    <Form action="http://localhost:8000/user/create" method="POST">
                        { this.errorDiv() }
                        ...
                        <Form.Group className="mb-3">
                            <Form.Control id="rep-password" type="password" placeholder="Repeat password" />
                        </Form.Group>
                        <div className="d-grid">
                            <Button id="btn-submit" type="submit" disabled>Sign up</Button>
                        </div>
                    </Form>
                </div>
